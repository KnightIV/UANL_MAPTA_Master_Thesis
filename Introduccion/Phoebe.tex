...
(\citetbooksection{phoebeScientificReference}{5.1.1})).
...
\begin{eqfloat}[!ht]
	...
	\begin{equation}
		\begin{split}
			f(\varrho_{\perp}) = \eqnmark[MyDarkRed]{omega1}{\frac{1}{\sqrt{\varrho_{\perp} + z^2}} + q \left(\frac{1}{\sqrt{\delta^2 + \varrho_{\perp}^2 + z^2 - 2 \varrho_{\perp} \delta \cos{\phi}}} - \frac{\varrho_{\perp} \cos{\phi}}{\delta^2} \right)} \\
			\eqnmark[MyDarkRed]{omega2}{+ \frac{F^2 \left(1 + q\right) \varrho_{\perp}^2}{2}} \eqnmark[MyDarkBlue]{omegaPol}{- \frac{1}{z_{\textrm{pol}}} - \frac{q}{\sqrt{z_{\textrm{pol}}^2 + \delta^2}}}
		\end{split}
	\end{equation}
	\annotate[yshift=0.8em]{above, left}{omega1}{Potencial de Roche $\Omega$}
...
\begin{figure}[!ht]
	\includegraphics[scale=0.4]{Introduccion/Figures/Figura Mallado Triangular_PHOEBE II Mesh.png}
...
la estrella compañera, $\omega(t)$ es la velocidad angular en el tiempo $t$, y $d_{\perp,\textrm{rot}}$ es la distancia del polo estelar hacia el eje
...
regiones de mayor gravedad superficial (en sistemas binarios suelen
...
fotón de atravesar la superficie estelar),
...
(\citeyearparen{lomb_least-squares_frequency_analysis_1976}, 
\citeyearparen{scargle_studies_time_series_fourier_1982}] el cual ajusta
...
Desarrollado por \citeyearparen{nelder_mead_simplex_1965}, el \textbf{simplex de
Nelder-Mead} (NMS por sus siglas en inglés) es un algoritmo heurístico que
explora el espacio de parámetros mediante un \textit{simplex}, un politopo
compuesto de varios vértices que representan distintos puntos en la función que
se busca optimizar. En el caso de PHOEBE, la función a optimizar es $\chi^2$;
para esto es necesario evaluar el ajuste en cada punto del simplex, lo cual
requiere calcular el modelo hacia adelante para cada punto de prueba. Esto se
demuestra con la siguiente modificación a la \refequation{ecuacionChi2}, visto
en la \refequation{ecuacionChi2Params}.
...
$\mathbf{p}$ representa el vector de parámetros del modelo, como la
...
\begin{figure}[!ht]
	\centering
	\xincludegraphics[scale=0.29, label=\textbf{(a)}, labelbox=true, pos=ne, fontsize=\Large]{Introduccion/Figures/Figura PHOEBE Priors Gaussian.png}
	\xincludegraphics[scale=0.29, label=\textbf{(b)}, labelbox=true, pos=ne, fontsize=\Large]{Introduccion/Figures/Figura PHOEBE Priors Uniform.png}
